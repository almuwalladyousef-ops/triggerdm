...
export async function sendPrivateReply(commentId, messages, token, igId, userInfo) {
  const text = buildPrivateReplyText(messages, userInfo)
  if (!commentId || !text) {
...
export async function sendPrivateReplyWithButton(commentId, promptText, buttonText, token, igId, userInfo) {
  ...
  const payload = {
    ...
    message: {
      text,
      quick_replies: [
        { content_type: 'text', title: buttonText || 'Get it!', payload: 'TRIGGER_STEP2' },
      ],
    },
  }
...
export async function sendDMToUser(userId, messages, token, igId, userInfo) {
  const text = buildPrivateReplyText(messages, userInfo)
  if (!userId || !text) throw new Error('Cannot send DM without userId and text')

  const payload = { recipient: { id: userId }, message: { text } }
